...
{
	struct wl_shm_buffer *buffer = wl_resource_get_data(resource);
...
{
	struct wl_shm_pool *pool = wl_resource_get_data(resource);
...
{
	return wl_resource_instance_of(buffer->resource, &shm_buffer_interface);
}
